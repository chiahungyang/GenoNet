...
"""
    _traverse(adjlist::Dict{T, Vector})::Function where {T}
    _traverse(adjlist::Vector{Vector})::Function
...
"""
function _traverse(adjlist::Dict{T, Vector})::Function where {T}
    function trav!(visited::Dict{T, Bool}, src::T)::Nothing
        let adjlist = adjlist
            haskey(adjlist, src) || throw(DomainError(src, "node not in the graph"))
...
function _traverse(adjlist::Vector{<:Vector})::Function
    function trav!(visited::BitVector, src::Integer)::Nothing
        let adjlist = adjlist
            checkbounds(Bool, adjlist, src) || throw(DomainError(src, "node not in the graph"))
            @inbounds begin
                visited[src] && return nothing
                visited[src] = true
                foreach(nei -> trav!(visited, nei), adjlist[src])
...
"""
    reachable(adjlist::Dict{T, Vector}, srcs::Vector{T})::Dict{T, Bool} where {T}
    reachable(adjlist::Vector{Vector}, srcs::Vector{Integer})::BitVector
...
```
"""
function reachable(adjlist::Dict{T, Vector}, srcs::Vector{T})::Dict{T, Bool} where {T}
...
function reachable(adjlist::Vector{<:Vector}, srcs::Vector{<:Integer})::BitVector
